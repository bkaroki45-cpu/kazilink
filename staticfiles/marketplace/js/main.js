...
async function pollNotificationStatus() {
    try {
        ...
        const data = await response.json();
        updateChatBadge(data.unread_chats || 0);

        const latest = data.latest;
        if (!latest || Notification.permission !== 'granted') return;
...
async function enablePushNotifications() {
    ...
    const registration = await navigator.serviceWorker.register('/static/marketplace/js/sw.js');
